  ...
  networking.nftables.tables.nixos-fw = {
    content = ''
      chain input_extra {
        ...
        tcp dport ${toString config.services.prometheus.exporters.unbound.port} ip6 saddr { 2001:67c:2ed8::4:1, fdeb:52c8:d094:a::14:1 } counter accept comment "prometheus-unbound-exporter: accept from elsa"
      }
    '';
  };
